...
async def _run_process_item(
    ...
    async with AsyncSessionLocal() as db:
        try:
            await process_item(db, user_id, user_item_id, url, max_video_sec)
        except Exception:
            logger.exception(
                "process_item failed: user_id=%s user_item_id=%s",
...
async def prepare_item_create(
    ...
    if existing is not None:
        ...
        needs = existing.parsed_at is None
