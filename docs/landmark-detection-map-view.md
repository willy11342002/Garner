...
class ContentLocation(Base):
    ...
    id: UUID (PK)
    content_id: UUID (FK → content_objects, ON DELETE CASCADE)
    name: str          # 地點名稱，如 "台北101"
    lat: float | None  # Geocoding 成功後填入
    lng: float | None
    source: str        # "metadata" | "ai"
    confirmed: bool    # 預設 False；使用者確認後為 True
...
for location in locations:
    ...
    insert ContentLocation(content_id, name, lat, lng, source, order_index)
```
...
- [ ] **DB Migration**：新增 `content_locations` 表與索引
...
- [ ] **CRUD**：新增 `crud/locations.py`（list by content_id、update confirmed、delete、bounding box query）
